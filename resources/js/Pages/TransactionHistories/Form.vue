...
const props = defineProps(['transactionHistory', 'id']);
...
const submitForm = () => {
    if (props.id) {
        form.put(route('transaction.update', {id: props.id}), {
            onFinish: (r) => {
                console.log(r);
                // form.reset('amount');
            },
...
<template>
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ props.id ? 'Atualizar' : 'Criar'}} Transação</h2>
        </template>
        <div class="">
            <div class="mx-auto">
